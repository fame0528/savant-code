...
- Exported `processToolCallBuffer` and state helpers so SDK consumers can strip `<savant-code_tool_call>` segments mid-stream.
...
- XML tool call filtering in stream chunks - filters out `<savant-code_tool_call>` tags while preserving response text
...
- `SavantClient.run` now does not return `null`. Instead, the `SavantClient.run(...).output.type` will be `'error'`.

## [0.3.0]

- New more intuitive interface for `SavantClient` and `SavantClient.run`.
...
- cwd is optional in the SavantClient constructor.
...
- You can now get an API key from the [Savant-Code website](https://www.savant-code.dev/profile?tab=api-keys)!
...
- `closeConnection` method in `SavantClient`
...
- `SavantClient` no longer requires binary to be installed
...
- Initial release of the Savant-Code SDK
- `SavantClient` class for interacting with Savant-Code agents
- `runNewChat` method for starting new chat sessions
- TypeScript support with full type definitions
- Support for all Savant-Code agent types
