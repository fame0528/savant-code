...
Official SDK for Savant-Code - AI coding agent and framework
...
- Create a Savant-Code account and get your [Savant-Code API key here](https://www.savant-code.dev/api-keys).
...
async function main() {
  const client = new SavantClient({
    ...
    // Get one here: https://www.savant-code.dev/api-keys
  ...
  const runState1 = await client.run({
    // The agent id. Any agent on the store (https://savant-code.dev/store)
...
```typescript
import { loadLocalAgents, SavantClient } from '@savant-code/sdk'
...
const client = new SavantClient({ apiKey: process.env.SAVANT_CODE_API_KEY })
...
Runs a Savant-Code agent with the specified options.
...
- **`projectFiles`** (object, optional): All the files in your project as a plain JavaScript object. Keys should be the full path from your current directory to each file, and values should be the string contents of the file. Example: `{ "src/index.ts": "console.log('hi')" }`. This helps Savant-Code pick good source files for context. Note: This parameter was previously named `allFiles` but has been renamed for clarity.
