...
export type SavantClientOptions = {
  apiKey?: string
...
function requireCwd(cwd: string | undefined, toolName: string): string {
  if (!cwd) {
    throw new Error(
      `cwd is required for the ${toolName} tool. Please provide cwd in SavantClientOptions or override the ${toolName} tool.`,
    )
  }
  return cwd
}
...
async function readFiles({
  ...
  override?: NonNullable<
    Required<SavantClientOptions>['overrideTools']['read_files']
  >
  fileFilter?: FileFilter
  cwd?: string
  fs: SavantFileSystem
...
async function handleToolCall({
  ...
  overrides: NonNullable<SavantClientOptions['overrideTools']>
