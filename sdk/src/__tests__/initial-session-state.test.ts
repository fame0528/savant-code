...
describe('Initial Session State', () => {
  ...
  beforeEach(() => {
    mockFs = {
      readFile: async (path: string) => {
        ...
        if (path.includes('.savant-codeignore')) {
          return ''
        }
        if (path.includes('.savant-codeignore')) {
          return ''
      ...
      exists: async (path: string) => {
        ...
        if (path.includes('.savant-codeignore')) return true
        if (path.includes('.savant-codeignore')) return true
      ...
      mkdir: async () => {},
      writeFile: async () => {},
    } as unknown as SavantFileSystem
  ...
  test('discovers project files automatically when projectFiles is undefined', async () => {
    ...
    mockFs.stat = (async (filePath: string) =>
      ...
      }) as MockStatResult & { size: number }) as SavantFileSystem['stat']
