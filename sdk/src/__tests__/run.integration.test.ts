...
describe('Prompt Caching', () => {
  it(
    ...
    async () => {
      if (!apiKey) {
        console.log(
          'Skipping prompt caching integration test: set SAVANT_CODE_API_KEY to run.\n' +
            'Example: SAVANT_CODE_API_KEY=your-key bun test src/__tests__/run.integration.test.ts',
        )
        return
      }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const client = new SavantClient({ apiKey, cwd: process.cwd() })
