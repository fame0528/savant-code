...
try {
  ...
  const { SavantClient } = require('@savant-code/sdk')
  console.log('âœ… Named destructuring successful:', typeof SavantClient)

  if (typeof SavantClient !== 'function') {
    throw new Error(
      `Expected SavantClient to be a function, got ${typeof SavantClient}`,
    )
  }
  ...
  const expectedExports = ['SavantClient', 'getCustomToolDefinition']
  ...
  const ClientFromDestructure = require('@savant-code/sdk').SavantClient
  const ClientFromDefault = require('@savant-code/sdk').SavantClient
  ...
  try {
    ...
    eval('import { SavantClient } from "@savant-code/sdk"')
  ...
  console.error('\nâŒ CommonJS import test failed:', error.message)
  process.exit(1)
}
