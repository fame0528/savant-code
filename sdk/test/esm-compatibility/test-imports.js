...
try {
  ...
  const { SavantClient } = await import('@savant-code/sdk');
  console.log('âœ… Named ESM import successful:', typeof SavantClient);
  
  if (typeof SavantClient !== 'function') {
    throw new Error(`Expected SavantClient to be a function, got ${typeof SavantClient}`);
  }
  ...
  const ClientFromNamed = namedModule.SavantClient;
  const ClientFromNamespace = SDK.SavantClient;
  ...
  try {
    eval('const { SavantClient } = require("@savant-code/sdk")');
  ...
  console.error('\nâŒ ESM import test failed:', error.message);
  process.exit(1);
}
