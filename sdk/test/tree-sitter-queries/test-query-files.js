...
async function testTreeSitterFunctionality() {
  console.log('ðŸ§ª Testing tree-sitter functionality with real source files...')
  ...
  try {
    ...
    console.log('âœ… initialSessionState completed successfully')
    ...
    console.log(
      `âœ… Generated token scores for ${Object.keys(fileTokenScores).length} files`,
    )
    ...
    for (const [fileName, expectedTokensForFile] of Object.entries(
      ...
      if (missingTokens.length > 0) {
        console.warn(
          `âš ï¸  Missing expected tokens in ${fileName}: ${missingTokens.join(', ')}`,
        )
        console.warn(
        ...
        console.log(
          `âœ… ${fileName}: Found all expected tokens (${foundTokens.length} total)`,
        )
      }
      ...
      console.log(
        `âœ… ${fileName}: ${nonZeroScores.length}/${tokenScores.length} tokens have positive scores`,
      )
    }
    ...
    if (tokenCallers && Object.keys(tokenCallers).length > 0) {
      console.log(
        `âœ… Generated token callers for ${Object.keys(tokenCallers).length} files`,
      )
    } else {
      console.log(
        'â„¹ï¸  No token callers generated (expected for single-file test)',
      )
    }
...
async function runAllTests() {
  try {
    ...
    console.log('\nðŸŽ‰ All tree-sitter query file tests passed!')
    console.log('   âœ… Query files are properly bundled')
    console.log('   âœ… Tree-sitter parsing works with real source files')
    console.log('   âœ… Token scores are generated correctly')
    process.exit(0)
  } catch (error) {
    console.error('\nâŒ Tree-sitter query file test failed:', error.message)
    if (error.stack) {
