## Savant-Code for Windows dev setup
...
You shouldn't need this - but just in case you stop using Shadow.tech, or make a new account, here's a guide on how to get from a blank Windows install to a Savant-Code install.
...
- Install Savant-Code: Run `npm i -g savant-code`
...
```powershell
PS C:\> savant-code
âŒ Failed to determine latest version
Please check your internet connection and try again
```

**Cause**:
Savant-Code checks GitHub for the latest release on first run. This fails when:
...
1. **Set the `HTTPS_PROXY` environment variable** (if behind corporate proxy):
   ...
   Savant-Code natively supports proxy environment variables. This is the recommended fix:
   ...
   **CMD:**
   ...
   To make it permanent, add `HTTPS_PROXY` to your Windows System Environment Variables (Settings â†’ System â†’ Advanced â†’ Environment Variables).
...
3. **Configure npm proxy** (for the `npm install` step only):
   ...
   ```
   Note: This only helps with `npm install`. Savant-Code's own downloads use `HTTPS_PROXY` instead.

4. **Disable VPN temporarily** or whitelist `registry.npmjs.org` and `savant-code.dev` in your firewall
...
**Reference**: Issue [#294](https://github.com/savant-code/savant-code/issues/294)
...
**Cause**:
Savant-Code requires bash for command execution. This error appears when:
...
1. **Install Git for Windows** (recommended):
   ...
   - This installs `bash.exe` which Savant-Code will automatically detect
...
**Reference**: Issue [#274](https://github.com/savant-code/savant-code/issues/274)
...
1. **Ensure Git for Windows is installed**:
   - Download from https://git-scm.com/download/win
   - Savant-Code uses bash.exe from Git for Windows for command execution
...
**Cause**:
When running Savant-Code in Git Bash (MINGW64), the `start` command is not available in PATH. The browser auto-open feature fails.

**Solutions**:

1. **Manually open the login URL** (easiest):
   - Savant-Code displays the login URL after the error
   - Copy the full URL starting with `https://savant-code.dev/login?auth_code=...`
...
**Reference**: Issue [#299](https://github.com/savant-code/savant-code/issues/299)

---

### Message: "Update available: error â†’ [version]"
...
- Savant-Code will automatically download the correct version

**What to do**:
- Wait for the download to complete: "Download complete! Starting Savant-Code..."
...
1. **Search existing issues**: https://github.com/savant-code/savant-code/issues
2. **Open a new issue**: https://github.com/savant-code/savant-code/issues/new
3. **Join Discord community**: https://savant-code.dev/discord
