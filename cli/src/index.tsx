...
import { resetSavantClient } from './utils/savant-code-client'
import { runStreamJsonMode } from './utils/stream-json-runner'
...
async function main(): Promise<void> {
  ...
  // through Parser.init end-to-end. Has to live BEFORE commander.parse() â€”
  ...
  if (process.argv.includes('--smoke-tree-sitter')) {
    ...
    try {
      ...
      if (effectiveBinary) {
        ...
        // Marker grepped by cli/scripts/smoke-binary.ts â€” keep this exact text.
        console.log(
        ...
        console.error(
          'tree-sitter smoke FAIL: no wasm available â€” pre-init published ' +
  ...
  if (isPublishCommand) {
    ...
    if (result.success && result.publisherId && result.agents) {
      logger.info(green('âœ… Successfully published:'))
      ...
      logger.error(red('âŒ Publish failed'))
  ...
  // Remove early handlers â€” proper cleanup handlers (with renderer access) take over
