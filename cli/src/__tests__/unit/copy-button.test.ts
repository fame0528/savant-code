...
describe('CopyButton - exported constants', () => {
  ...
  test('icon constants are defined', () => {
    expect(COPY_ICON_COLLAPSED).toBe('âŽ˜')
    expect(COPY_ICON_EXPANDED).toBe('[âŽ˜ copy]')
    expect(COPY_ICON_COPIED).toBe('[âœ” copied]')
  })
})
