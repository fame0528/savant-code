...
Comprehensive testing suite for the Savant-Code CLI using tmux for interactive terminal emulation.
...
- âœ… Project-wide (works in any package)
...
```
âš ï¸  tmux not found but required for integration/E2E tests
```
...
```
âœ“ Build SDK for E2E tests: cd sdk && bun run build [skip]
```
