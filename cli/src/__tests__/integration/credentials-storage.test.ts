...
describe('Credentials Storage Integration', () => {
  ...
  beforeEach(() => {
    ...
    tempConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'savant-code-test-'))
  ...
  describe('P0: File System Operations', () => {
    ...
    test('should use savant-code-dev directory in development environment', async () => {
      ...
      await mockModule('@savant-code/common/env', () => ({
        env: { NEXT_PUBLIC_SC_ENVIRONMENT: 'dev' },
      }))

      // Call real getConfigDir to verify it includes '-dev'
      const configDir = authModule.getConfigDir()
      expect(configDir).toEqual(
        path.join(os.homedir(), '.config', 'savant-code-dev'),
      )
    })
