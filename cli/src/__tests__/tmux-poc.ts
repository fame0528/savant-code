...
async function testCLIWithTmux() {
  ...
  console.log('ðŸš€ Starting tmux-based CLI test...')
  console.log(`ðŸ“¦ Session: ${sessionName}`)

  // 1. Check if tmux is installed
  if (!isTmuxAvailable()) {
    console.error('âŒ tmux not found')
    console.error('\nðŸ“¦ Installation:')
    ...
    console.error(
      '\nâ„¹ï¸  This is just a proof-of-concept. See the documentation for alternatives.',
    )
    process.exit(1)
  }
  ...
  try {
    ...
    console.log(`âœ… tmux is installed: ${version.trim()}`)

    // 2. Create new detached tmux session running the CLI
    console.log('\nðŸ“º Creating tmux session...')
    await tmux([
      'new-session',
      '-d',
      '-s',
    ...
    checks.forEach(({ text, pass }) => {
      console.log(
        `  ${pass ? 'âœ…' : 'âŒ'} Contains "${text}"${pass ? '' : ' - NOT FOUND'}`,
      )
    })
    ...
    */
    ...
    try {
      ...
      console.log('âš ï¸  Session may have already exited')
    }
  }
}
