...
export function handleInitializationFlowLocally(): {
  ...
  if (existsSync(knowledgePath)) {
    messages.push(`ðŸ“‹ \`${PRIMARY_KNOWLEDGE_FILE_NAME}\` already exists.`)
  } else {
  ...
  if (existsSync(agentsDir)) {
    messages.push('ðŸ“‹ `.agents/` already exists.')
  } else {
  ...
  if (existsSync(agentsTypesDir)) {
    messages.push('ðŸ“‹ `.agents/types/` already exists.')
  } else {
  ...
  for (const { fileName, source } of COMMON_TYPE_FILES) {
    ...
    if (existsSync(targetPath)) {
      messages.push(`ðŸ“‹ \`.agents/types/${fileName}\` already exists.`)
      continue
    }
