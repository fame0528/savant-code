...
export async function generateLoginUrl(
  ...
  if (!response.ok) {
    logger.error(
      ...
      'âŒ Failed to request login URL',
    )
  ...
  if (!response.data) {
    logger.error(
      { status: response.status },
      'âŒ Empty response from login URL',
    )
    throw new Error('Failed to get login URL')
  }
...
export async function pollLoginStatus(
  ...
  while (true) {
    if (shouldContinue && !shouldContinue()) {
      logger.warn('ðŸ›‘ Polling aborted by caller')
      return { status: 'aborted' }
    }

    if (now() - startTime >= timeoutMs) {
      logger.warn('âŒ›ï¸ Login polling timed out')
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status !== 401) {
          logger.warn(
            ...
            'âš ï¸ Unexpected status while polling',
          )
        }
      ...
      logger.error(
        ...
        'ðŸ’¥ Network error during login status polling',
      )
      await sleep(intervalMs)
      continue
    }
