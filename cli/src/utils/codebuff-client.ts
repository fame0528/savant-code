...
let clientInstance: SavantClient | null = null
...
/**
 * Reset the cached SavantClient instance.
 * This should be called after login to ensure the client is re-initialized with new credentials.
 */
export function resetSavantClient(): void {
  clientInstance = null
}

export async function getSavantClient(): Promise<SavantClient | null> {
  if (!clientInstance) {
    ...
    try {
      ...
      clientInstance = new SavantClient({
        apiKey,
        cwd: projectRoot,
        agentDefinitions,
        logger,
      ...
      logger.error(error, 'Failed to initialize SavantClient')
      return null
    }
  }
