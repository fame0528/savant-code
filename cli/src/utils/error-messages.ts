 ...
 *
 * Example output:
 *   "âš ï¸ Network error: Server error. Please try again later. â€¢ 3 messages will retry when connection is restored"
 */
export function formatRetryBannerMessage(error: unknown, pendingCount: number): string {
  ...
  const suffix =
    pendingCount > 0
      ? ` â€¢ ${pendingCount} message${pendingCount === 1 ? '' : 's'} will retry when connection is restored`
      : ''

  return `âš ï¸ ${formatted}${suffix}`
}
