...
export async function validateApiKey({
  ...
  try {
    ...
    if (!authResult) {
      logger.error('âŒ API key validation failed - invalid credentials')
    ...
    if (isAuthenticationError(error)) {
      logger.error('âŒ API key validation failed - authentication error')
    ...
    if (statusCode !== undefined && isRetryableStatusCode(statusCode)) {
      logger.error(
        ...
        'âŒ API key validation failed - network error',
      )
      // Rethrow the original error to preserve statusCode for higher layers
      throw error
    }
    ...
    logger.error(
      ...
      'âŒ API key validation failed - unknown error',
    )
