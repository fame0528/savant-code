 ...
 * 3. Attempt run B â€” verify it's blocked (chain lock held)
...
describe('CLI-level race condition: abort run A, attempt run B before A resolves', () => {
  ...
  test('run B can proceed immediately after abort (chain lock released by abort handler)', () => {
    ...
    // --- PHASE 3: User types run B â€” verify it's UNBLOCKED ---
    ...
    // Run B can proceed immediately â€” this is the core fix.
  ...
  test('reject-after-abort must not run handleRunError cleanup that could clobber run B', () => {
    ...
    if (!abortController.signal.aborted) {
      // This should NOT execute â€” handleRunError would clobber run B's state
      handleRunError({
        error,
  ...
  test('handleRunError WOULD clobber run B state if called without abort guard (documents why guard is needed)', () => {
    ...
    // handleRunError resets ALL shared state â€” this would clobber run B
  ...
  test('full two-run lifecycle with shared streamRefs: run A abort â†’ run B starts immediately', () => {
