...
export const useSendMessage = ({
  ...
  const sendMessage = useCallback<SendMessageFn>(
    async ({ content, agentMode, postUserMessage, attachments }) => {
      ...
      try {
        ...
        setMessages((prev) => [
          ...
          createErrorChatMessage(
            'âš ï¸ Failed to prepare message. Please try again.',
          ),
        ])
        resetEarlyReturnState({
          setCanProcessQueue,
      ...
      // Get SDK client
      const client = await getSavantClient()
      ...
      if (!client) {
        ...
        setMessages((prev) => [
          ...prev,
          createErrorChatMessage(
            `âš ï¸ Unable to connect to ${brandName}. Please check your authentication and try again.`,
          ),
        ])
        await yieldToEventLoop()
        setTimeout(() => scrollToLatest(), 0)
