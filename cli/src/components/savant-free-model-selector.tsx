...
// inline â€” "N of M used Â· resets in â€¦" â€” once any session is spent (turning
...
const FOCUS_CUE = 'Press Enter â†µ'
 ...
 *     a model is their explicit commitment to enter â€” this triggers the POST.
 ...
 * Space) commits the focused row â€” or, on the toggle, expands/collapses the
...
export const SavantFreeModelSelector: React.FC<SavantFreeModelSelectorProps> = ({
  ...
  // contentMaxWidth (not terminalWidth) is the real budget â€” the parent
  ...
  // single "other" model (limited tier) we just show both â€” a "see 1 more
  ...
  // selection IS the recommended model â€” a returning user whose preference is a
  ...
  // Mirror the expanded state up to the waiting-room screen (collapsed â†’ it
  ...
  // Keyboard cursor â€” separate from the actually-selected model so that
  ...
  useEffect(() => {
    ...
    // the server will immediately reject. In-memory only â€” the user's saved
  ...
  // Â· resets in â€¦". All premium models share one pool; the server replicates
  // the same snapshot under every model id, so any entry has the right count.
  // The count shows from the start â€” even at "0 of M" â€” so full-access users
  // can see the daily pool and reset cadence before they spend anything; it
  // turns amber when the pool is exhausted â€” the same moment the premium rows
  // grey out â€” so the header explains why they're disabled. (The PREMIUM
  ...
  // across all rows) followed by a details column (tagline Â· warning Â·
  ...
  const {
    ...
    // the "Press Enter â†µ" gutter. Folding that into the max means longer rows
    // (e.g. DeepSeek Pro's data-collection warning) keep their natural width â€”
    ...
    // length the gutter was reserved for â€” one formula, no reserve/consume drift.
  ...
  const renderModelButton = (
    ...
    // saved/committed pick is not shown separately â€” it just sets where
    ...
    // Clickable whenever picking would actually do something â€” i.e.
    ...
    // itself stays the normal foreground color so it doesn't shout â€” the
    // border and arrow do the highlighting. Off-focus rows are default.
    const indicator = isFocused ? 'â€º' : ' '
    const fgColor = canJoin ? theme.foreground : theme.muted
    const mutedColor = theme.muted
    const warningColor = theme.secondary

    // Focused row gets the bright primary border (and arrow). Every other row â€”
    // including the recommended card when the cursor has moved elsewhere â€” stays
    ...
    // Right-aligned "Press Enter â†µ" cue on the focused recommended row only.
    // Right-align against recommendedOneLineLen â€” the exact length the gutter was
    // reserved against above â€” so reserve and consume can't drift. The reservation
    ...
    return (
      ...
      >
        <text>
          ...
          {wrapDetails ? (
            <span fg={mutedColor}> Â· {model.tagline}</span>
          ) : (
            <>
              <span fg={mutedColor}>{namePadding + model.tagline}</span>
              {hasWarning && <span fg={warningColor}> Â· {model.warning}</span>}
              {hasHours && (
                <span fg={mutedColor}> Â· {deploymentAvailabilityLabel}</span>
              )}
              {showCue && (
        ...
        {wrapDetails && (hasWarning || hasHours) && (
          <text>
            ...
            {hasWarning && hasHours && <span fg={mutedColor}> Â· </span>}
            {hasHours && (
  ...
  const sectionsContent = sections.map((section) => (
    ...
    >
      {/* wrapMode 'none' pins headers to one row â€” the offset math above
        ...
        <text style={{ fg: theme.muted, wrapMode: 'none' }}>
          ...
          {section.key === 'premium' && (
            <span fg={premiumExhausted ? theme.secondary : theme.muted}>
              {' '}
              Â· {formatSessionUnits(premiumUsed)} of{' '}
              {SAVANT_FREE_PREMIUM_SESSION_LIMIT} used
            </span>
          )}
          {section.key === 'premium' && premiumResetCountdown && (
            <span fg={theme.muted}> Â· resets in {premiumResetCountdown}</span>
          )}
        </text>
      )}
  ...
  const toggleLabel = expanded
    ? 'â†‘  Show fewer'
    : `â†“  See all ${availableModels.length} models`
