...
export const ChatInputBar = ({
  ...
  const mentionMenuFooter = isShallowScanRoot(tryGetProjectRoot())
    ? `Files shown up to ${SHALLOW_SCAN_MAX_DEPTH} levels deep â€” open a project folder for full results`
    : undefined
      ...
      <>
        ...
        >
          ...
          {/* In default modes the compact box has no border or label, so it can
              ...
              that it's a focusable input â€” costs no extra height. */}
          {!modeConfig.label && !modeConfig.icon && (
            <box style={{ flexShrink: 0 }}>
              <text style={{ fg: theme.primary }}>â¯</text>
            </box>
          )}
          <MultilineInput
            value={inputValue}
