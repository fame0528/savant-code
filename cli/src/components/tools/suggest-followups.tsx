...
const FollowupLine = ({
  ...
  const truncatedPrompt = showDescription
    ? (() => {
        ...
        return followup.prompt.length > availableWidth
          ? followup.prompt.slice(0, availableWidth - 1) + 'â€¦'
          : followup.prompt
      })()
    : ''
  ...
  const paddingSpaces = showDescription
    ? ' '.repeat(Math.max(0, labelColumnWidth - 2 - labelLength)) // -2 for "â†’ " prefix
    : ''
      ...
      <box style={{ flexDirection: 'row', width: '100%' }}>
        ...
        >
          <text style={{ wrapMode: hasLabel ? 'none' : 'word' }}>
            <span fg={iconColor}>{isClicked ? 'âœ“' : 'â†’'}</span>
            <span
              fg={labelColor}
...
const PastFollowupsToggle = ({
  ...
  const toggleIndicator = isExpanded ? 'â–¾' : 'â–¸'
...
const SuggestFollowupsItem = ({
  ...
  // Width = "â†’ " (2 chars) + max label/prompt length + "  " spacing (2 chars)
