...
async function main() {
  const DEBUG = false
  if (DEBUG) {
    console.log('ðŸ” DEBUG: Scanning agents/ directory...')
  }
  ...
  if (DEBUG) {
    console.log(`ðŸ“ DEBUG: Found ${tsFiles.length} TypeScript files`)
  }
  ...
  for (const filePath of tsFiles) {
    ...
    if (definition) {
      ...
      loadedCount++
      if (DEBUG) {
        console.log(`  âœ… DEBUG: ${definition.id} (${relativePath})`)
      }
    } else {
      skippedCount++
      if (DEBUG) {
        console.log(`  â­ï¸ DEBUG: Skipped: ${relativePath} (no valid default export)`)
      }
    }
  }

  if (DEBUG) {
    console.log(`\nðŸ“¦ DEBUG: Loaded ${loadedCount} agents, skipped ${skippedCount} files`)
  }
  ...
  if (DEBUG) {
    console.log(`\nâœ¨ DEBUG: Generated ${OUTPUT_FILE}`)
