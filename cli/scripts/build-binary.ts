...
async function main() {
  ...
  // retrieval mechanism we tried â€” `with { type: 'file' }`, base64 string
  // literals, chunked base64, function-wrapped chunked base64 â€” got
  ...
  logAlways(`Copied tree-sitter.wasm sibling: ${sourceWasm} â†’ ${siblingWasm}`)
  ...
  logAlways(
    `âœ… Built ${outputFilename} (${getCliTargetLabel(targetInfo)})`,
  )
}
 ...
 * layout â€” bun hoists differently across platforms and `bun install`
