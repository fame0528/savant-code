 ...
 * screen appear" assertion catches *any* startup failure â€” known fatals,
...
// Fatal markers we already know about â€” kept for nicer error messages on
...
// *before any output* â€” the renderer never reaches its first write, so the
...
// hang doesn't fail the build. Regression detection is preserved â€” a known
...
async function main(): Promise<void> {
  ...
  console.log(`smoke-binary: spawning ${binary} for ${runSeconds}sâ€¦`)
  ...
  for (let attempt = 1; attempt <= MAX_BOOT_ATTEMPTS; attempt++) {
    console.log(
      `smoke-binary: boot attempt ${attempt}/${MAX_BOOT_ATTEMPTS} (running ${runSeconds}s)â€¦`,
    )
    ...
    if (outcome.kind === 'fatal') {
      // Deterministic crash â€” a known fatal marker is a real regression, not a
      // flaky hang, so fail immediately without burning the remaining retries.
      console.error(
        `smoke-binary: FAIL â€” output matched ${outcome.pattern} (exit code ${outcome.exitCode}).`,
      )
    ...
    console.error(
      `smoke-binary: attempt ${attempt}/${MAX_BOOT_ATTEMPTS} produced no boot signal ` +
        ...
        (attempt < MAX_BOOT_ATTEMPTS ? '; retryingâ€¦' : '.'),
    )
  }

  console.error(
    `smoke-binary: FAIL â€” binary never reached a known boot screen across ` +
      `${MAX_BOOT_ATTEMPTS} attempts â€” checked ${BOOT_SIGNAL_PATTERNS.length} patterns ` +
