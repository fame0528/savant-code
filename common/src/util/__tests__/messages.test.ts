...
describe('convertCbToModelMessages', () => {
  ...
  describe('lone surrogate sanitization', () => {
    // A lone (unpaired) UTF-16 surrogate â€” e.g. produced by slicing a file read
    // in the middle of an emoji â€” serializes to a syntactically-valid \uXXXX
    ...
    const LONE_HIGH = '\uD83D' // high surrogate of ðŸ˜€ with the low half dropped
    const REPLACEMENT = 'ï¿½'
    ...
    it('leaves valid surrogate pairs (emoji) intact', () => {
      const result = convertCbToModelMessages({
        messages: [userMessage('keep the ðŸ˜€ emoji')],
        includeCacheControl: false,
      })
      assertWellFormed(result)
      expect(JSON.stringify(result)).toContain('keep the ðŸ˜€ emoji')
    })
  })
