...
function assistantToSavantMessage(
...
function convertToolMessage(message: Message): ModelMessageWithAuxiliaryData[] {
  ...
  if (message.role === 'assistant') {
    ...
    return message.content.map((c) => {
      return assistantToSavantMessage({
        ...message,
        content: c,
      })
    })
 ...
 * strict server-side parsers â€” notably Rust's serde_json, used by
 * OpenAI/OpenRouter/Anthropic â€” reject the whole request body with
...
function toWellFormedString(str: string): string {
  return str.replace(LONE_SURROGATE_REGEX, 'ï¿½')
}
