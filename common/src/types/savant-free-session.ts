...
export type Savant-FreeSessionServerResponse =
  ...
  | ({
      ...
      position: number
      queueDepth: number
      /** Current depth of every model's queue. Retained for diagnostics â€”
  ...
  | {
       ...
       *  room only to be rejected on their first chat request. Terminal â€”
  ...
  | {
       ...
       *  15s admission tick's `evictBanned` sweeps them). Terminal â€” CLI
  ...
  | {
      ...
      windowHours: number
      /** Session units since today's Pacific reset â€” will be â‰¥ limit. */
      recentCount: number
