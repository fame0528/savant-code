...
import type { SavantFreeModelOption } from '../constants/savant-free-models'
import { minimaxModels } from '../constants/model-config'

const MINIMAX_M3_MODEL_ID = minimaxModels.minimaxM3

describe('SavantFree model availability', () => {
  ...
  test('only the DeepSeek family is trace-stored in free mode; M3 has no warning', () => {
    ...
    expect(isSavantFreeTracedModelId(SAVANT_FREE_DEEPSEEK_V4_PRO_MODEL_ID)).toBe(true)
  ...
  test('trace storage is one source of truth with the data-collection warning', () => {
    ...
    const models: readonly SavantFreeModelOption[] = SUPPORTED_SAVANT_FREE_MODELS
  ...
  test('MiniMax M2.7 is legacy: hidden from pickers but still served for old clients', () => {
    ...
    expect(
      getSavantFreeModelsForAccessTier('full').map((m) => m.id),
    ).not.toContain(SAVANT_FREE_MINIMAX_MODEL_ID)
    expect(isSavantFreeModelId(SAVANT_FREE_MINIMAX_MODEL_ID)).toBe(false)
    expect(isSupportedSavantFreeModelId(SAVANT_FREE_MINIMAX_MODEL_ID)).toBe(true)
    // Old clients with a saved M2.7 selection must still be admitted.
    expect(
      isSavantFreeModelAllowedForAccessTier(SAVANT_FREE_MINIMAX_MODEL_ID, 'full'),
    ).toBe(true)
    expect(
      resolveSavantFreeModelForAccessTier(SAVANT_FREE_MINIMAX_MODEL_ID, 'full'),
  ...
  test('limited access exposes DeepSeek V4 Flash and non-Pro MiMo 2.5', () => {
    ...
    expect(
      isSavantFreeModelAllowedForAccessTier(SAVANT_FREE_MINIMAX_MODEL_ID, 'limited'),
    ).toBe(false)
    expect(
    ...
    expect(
      resolveSavantFreeModelForAccessTier(SAVANT_FREE_MINIMAX_MODEL_ID, 'limited'),
  ...
  test('recommends an unlimited, in-tier model for the picker hero', () => {
    ...
    expect(isSavantFreePremiumModelId(getRecommendedSavantFreeModelId('full'))).toBe(
      false,
    )
  ...
  test('full-access SavantFree models can spawn the gemini-thinker subagent', () => {
    ...
    expect(canSavantFreeModelSpawnGeminiThinker(SAVANT_FREE_MINIMAX_MODEL_ID)).toBe(
      false,
    )
  ...
  test('does not support GLM 5.1 for SavantFree sessions', () => {
