...
import { SavantClient, getUserCredentials } from '@savant-code/sdk'
...
function printTaskResult(taskResult: {
  ...
  console.log('\n' + '='.repeat(80))
  console.log('ðŸ“‹ GENERATED TASK')
  console.log('='.repeat(80))
  console.log(`\nðŸ·ï¸  Task ID: ${taskResult.id}\n`)
  console.log(`ðŸ’­ Reasoning:\n${taskResult.reasoning}\n`)
  console.log(`ðŸ“ Spec:\n${taskResult.spec}\n`)
  console.log(`ðŸ’¬ Prompt:\n${taskResult.prompt}\n`)
  console.log(`ðŸ“ Supplemental Files (${taskResult.supplementalFiles.length}):`)
...
function savePartialResults(partialPath: string, evalData: EvalDataV2): void {
  ...
  console.log(`ðŸ’¾ Saved partial results to ${partialPath}`)
}
...
export async function generateEvalFileV2({
  ...
  const processCommit = async (
    ...
    return await withTestRepoAndParent(
      ...
      async (repoPath, commitSha, parentSha) => {
        ...
        if (removed.length > 0) {
          console.log(`âš ï¸  Filtered out ${removed.length} supplementalFiles that don't exist at parentSha:`)
  ...
  console.log(`\nâœ… Eval data written to ${finalOutputPath}`)

  if (fs.existsSync(partialOutputPath)) {
    fs.unlinkSync(partialOutputPath)
    console.log(`ðŸ—‘ï¸  Removed partial file: ${partialOutputPath}`)
  }
}
