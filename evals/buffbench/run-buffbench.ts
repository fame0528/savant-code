  ...
  for (const bin of binInstalls) {
    try {
      ...
      if (fs.existsSync(fullBinPath)) {
        ...
        console.log(`âœ“ ${bin.name} installed at ${fullBinPath}`)
      } else {
        console.warn(
...
export async function runBuffBench(options: {
  ...
  for (const agentData of Object.values(results)) {
    ...
    // Calculate average excluding huge failures (scores â‰¤1.0)
  ...
  if (binsTempDir) {
    try {
      ...
      console.log(`âœ“ Cleaned up binaries installation at ${binsTempDir}.`)
    } catch (error) {
  ...
  for (const [agentId, data] of Object.entries(results)) {
    ...
    console.log(
      `  Average Score (excluding failures â‰¤1.0): ${data.averageScoreExcludingFailures.toFixed(2)}/10 (${runsExcludingFailures.length}/${validRuns.length} runs)`,
    )
