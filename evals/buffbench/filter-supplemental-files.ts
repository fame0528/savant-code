...
function processEvalFile(
  ...
  if (!repoUrl) {
    console.log(`âš ï¸  No repoUrl found in eval file, skipping`)
  ...
  try {
    ...
    console.log(`âš ï¸  Failed to clone repo ${repoUrl}: ${error}`)
    return { totalRemoved: 0, commitUpdates: [] }
  }
...
function main() {
  ...
  for (const evalFile of evalFiles) {
    ...
    if (!fs.existsSync(evalFilePath)) {
      console.log(`âš ï¸  File not found: ${evalFile}`)
      continue
    }
    ...
    if (commitUpdates.length === 0) {
      console.log('âœ… No invalid supplementalFiles found')
    } else {
