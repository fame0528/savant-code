...
- **Works with any TUI app** - Savant-Code, Claude Code, Codex, or any custom TUI application
...
### Testing Savant-Code (Default)

```bash
# Start a test session (defaults to Savant-Code dev server)
...
```bash
# Build the binary first (Savant-Code example)
...
SAVANT_CODE_BINARY=./cli/bin/savant-code ./scripts/tmux/tmux-cli.sh start
```
...
# Default settings (Savant-Code dev server)
...
```
debug/tmux-sessions/
â””â”€â”€ tui-test-1234567890/
    â”œâ”€â”€ session-info.yaml             # Session metadata (YAML format)
    â”œâ”€â”€ commands.yaml                 # Log of all commands sent (YAML array)
    â”œâ”€â”€ capture-001-initial-state.txt # Captures with YAML front-matter
    â”œâ”€â”€ capture-002-after-status.txt
    â””â”€â”€ capture-003-after-prompt.txt
```
...
- `@savant-code-tester` - Tests the Savant-Code CLI
