 ...
 * as spawnParams â†’ toolCallParams â†’ generator params, matching base2 exactly.
 */
...
describe('Context Pruning Threshold E2E', () => {
  ...
  it(
    ...
    async () => {
      ...
      //   1. Calibration run with 200k limit (no pruning) â†’ measure TRUE token count
      //   2. Test run with 100k limit â†’ check if pruning triggers
      //
      ...
      // actual tokens â€” close to the 100k limit but safely under with accurate counting.
      //
      // Accurate counting:  ~90k < 100k â†’ no pruning in either run âœ“
      // 30% buffer:         ~90k reported as ~117k â†’ premature pruning in 100k run âœ—
      // Local fallback:     ~90k reported as ~135k+ â†’ premature pruning in 100k run âœ—
      ...
      // =========================================================================
      // Step 1: CALIBRATION RUN â€” measure true token count with 200k limit (no pruning)
      // =========================================================================
      ...
      // =========================================================================
      // Step 2: TEST RUN â€” same content with 100k limit
      // =========================================================================
      const sessionState = await initialSessionState({})
      const runStateWithMessages = withMessageHistory({
        runState: {
      ...
      if (trueTokenCount < MAX_CONTEXT_LENGTH && pruningResult.wasPruned) {
        console.error(
          `  âŒ BUG DETECTED: True tokens (${trueTokenCount}) < limit (${MAX_CONTEXT_LENGTH}), ` +
        ...
        console.log(
          `  âœ… No bug: True tokens (${trueTokenCount}) < limit (${MAX_CONTEXT_LENGTH}), ` +
            `no pruning occurred.`,
        )
      } else {
        console.log(
          `  âš ï¸ Content too large: True tokens (${trueTokenCount}) >= limit (${MAX_CONTEXT_LENGTH}). ` +
      ...
      console.log(
        ...
        ratio.toFixed(2),
        '(expected: 1.0-1.3, 30% bug â†’ 1.3+, fallback â†’ 1.5+)',
      )
