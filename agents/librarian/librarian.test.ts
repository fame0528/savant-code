...
import { SavantClient, loadLocalAgents } from '@savant-code/sdk'
...
async function runTask(
  ...
  console.log(`\n${'â”€'.repeat(60)}`)
  console.log(`Result: ${success ? 'âœ… SUCCESS' : 'âŒ FAILURE'}`)
...
async function main() {
  ...
  for (const r of results) {
    console.log(`  ${r.success ? 'âœ…' : 'âŒ'} ${r.name} â†’ ${r.traceFile}`)
